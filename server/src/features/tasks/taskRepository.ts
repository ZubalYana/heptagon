...
export const taskRepository = {
  ...
  async toggle(taskId: string, dayId: string) {
    ...
    if (task.repetition === null) {
      ...
      updated = await Task.findByIdAndUpdate(taskId, updateOperation, {
        returnDocument: "after",
      });
    }
  ...
  async edit(taskId: string, text: string, priority: Priority, repetition: Repetition){
  ...
  async addSubtask(taskId: string, taskDate: string, subtaskText: string){
    ...
    parentalTask.subtasks.push({subtaskText})

    if(parentalTask.repetition){
        newDateArray = parentalTask.completedDates.includes(taskDate)?
        parentalTask.completedDates.filter((d)=>d!=taskDate) : parentalTask.completedDates
  ...
  async toggleSubtask(taskId: string, subtaskId: string, date: string){
    const parentalTask = await Task.findById(taskId);
    if(!parentalTask) throw new Error("Parental task not found in database records.")
    ...
    if(parentalTask.repetition === null){
        ...
        const uncompletedSubtasks = parentalTask.subtasks.filter((s)=>s.completed == false);
        ...
        subtask.completedDates = subtask.completedDates.filter((d) => d !== date) :
        subtask.completedDates.push(date)

        const uncompletedSubtasks = parentalTask.subtasks.filter((s)=>!s.completedDates.includes(date))
        if(uncompletedSubtasks.length === 0){
            ...
            parentalTask.completedDates.includes(date)?
            parentalTask.completedDates = parentalTask.completedDates.filter((d)=>d!=date) :
            parentalTask.completedDates = parentalTask.completedDates
        }
    }
  ...
  async editSubtask(taskId: string, subtaskId: string, text: string){
    const parentalTask = await Task.findOne({_id: taskId});
    if(!parentalTask) throw new Error("Parental task not found in database records.")
  ...
  async deleteSubtask(taskId: string, subtaskId: string){
    const parentalTask = await Task.findById({_id: taskId});
    if(!parentalTask) throw new Error("Parental task not found in database records.")
